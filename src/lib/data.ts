...
const fetchFromStrapi = async <T>(path: string, fallbackData: T): Promise<T> => {
    noStore(); 
    
    if (!STRAPI_API_TOKEN) {
        console.warn("STRAPI_API_TOKEN is not set. Falling back to mock data for path:", path);
        return fallbackData;
    }
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`Strapi fetch failed for path: ${path} with status: ${res.status}. Falling back to mock data.`);
            return fallbackData;
        }
        ...
        console.error(`Strapi connection error for path: ${path}. Falling back to mock data.`, error);
        return fallbackData;
    }
};
...
export const getProjects = async (): Promise<Project[]> => {
    return fetchFromStrapi<Project[]>('projects?populate=*', sampleProjects);
};

export const getProjectBySlug = async (slug: string): Promise<Project | null> => {
    const fallback = sampleProjects.find(p => p.slug === slug) || null;
    const projects = await fetchFromStrapi<Project[]>(`projects?filters[slug][$eq]=${slug}&populate=*`, fallback ? [fallback] : []);
    return projects.length > 0 ? projects[0] : null;
};

export const getServices = async (): Promise<Service[]> => {
    return fetchFromStrapi<Service[]>('services?populate=*', sampleServices);
};

export const getServiceBySlug = async (slug: string): Promise<Service | null> => {
    const fallback = sampleServices.find(s => s.slug === slug) || null;
    const services = await fetchFromStrapi<Service[]>(`services?filters[slug][$eq]=${slug}&populate=*`, fallback ? [fallback] : []);
    return services.length > 0 ? services[0] : null;
};

export const getTestimonials = async (): Promise<Testimonial[]> => {
    return fetchFromStrapi<Testimonial[]>('testimonials?populate=*', sampleTestimonials);
};

export const getArticles = async (): Promise<Article[]> => {
    const publishedArticles = sampleArticles.filter(a => a.status === 'published');
    return fetchFromStrapi<Article[]>('articles?filters[status][$eq]=published&populate=*', publishedArticles);
};

export const getArticleBySlug = async (slug: string): Promise<Article | null> => {
    const fallback = sampleArticles.find(a => a.slug === slug && a.status === 'published') || null;
    const articles = await fetchFromStrapi<Article[]>(`articles?filters[slug][$eq]=${slug}&filters[status][$eq]=published&populate=*`, fallback ? [fallback] : []);
    return articles.length > 0 ? articles[0] : null;
};

export const getSettings = async (): Promise<SiteSettings> => {
    return fetchFromStrapi<SiteSettings>('setting?populate=socials', sampleSettings);
};
...
export const addMessage = async (message: Omit<Message, 'id' | 'submittedAt'>) => {
    console.log("Submitting message to Strapi (or mock):", message);
    await new Promise(resolve => setTimeout(resolve, 500)); 
    return { id: new Date().toISOString() };
};

export const addIntake = async (intake: NewIntake) => {
    console.log("Submitting intake to Strapi (or mock):", intake);
    await new Promise(resolve => setTimeout(resolve, 500));
    return { id: new Date().toISOString() };
};
