...
export const StaggeredMenu = ({
  position = 'right',
  colors = ['hsl(var(--primary) / 0.8)', 'hsl(var(--primary))'],
  items = [],
  socialItems = [],
  displaySocials = true,
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set([panel, ...preLayerElsRef.current], { xPercent: offscreen });
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const layerStates = preLayerElsRef.current.map(el => ({ el, start: Number(gsap.getProperty(el, 'xPercent')) }));
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    textCycleAnimRef.current = gsap.to(inner, { yPercent: targetY, duration: 0.4, ease: 'power2.out' });
  }, []);
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {colors.slice(0, 2).map((c, i) => <div key={i} className="sm-prelayer" style={{ background: c }} />)}
      </div>
