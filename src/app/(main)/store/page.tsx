    ...
    <div className="w-full bg-black text-white font-tech">
      <main>
        <header className="px-5 md:px-10 py-16 md:py-28 border-b border-white/20 relative">
            <div className="text-xs text-neutral-400 mb-5 tracking-wider flex gap-5">
            ...
            <div className="flex h-full overflow-x-auto whitespace-nowrap scrollbar-hide">
                {categories.map(cat => (
                    <button 
                        ...
                        className={cn(
                            "bg-transparent border-r border-white/20 text-neutral-400 font-tech text-sm px-4 md:px-8 h-full uppercase transition-all duration-200 hover:text-white hover:bg-white/5 flex-shrink-0",
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full">
            {paginatedTemplates.map((template) => (
              <div key={template.id} className="group product-card block border-b border-r border-white/20 bg-black">
                <div className="relative">
                    {template.bestSeller && (
                      <div className="absolute top-2 left-2 z-10 bg-black border border-white/50 text-white px-2 py-0.5 text-[10px] md:top-4 md:left-4 md:px-2.5 md:py-1 md:text-xs">BEST SELLER</div>
                    )}
                    {!template.bestSeller && template.isNew && (
                      <div className="absolute top-2 left-2 z-10 bg-black border border-white/50 text-white px-2 py-0.5 text-[10px] md:top-4 md:left-4 md:px-2.5 md:py-1 md:text-xs">NEW</div>
                    )}
                ...
                <div className="p-4 md:p-8 flex flex-col justify-between flex-grow">
                    <div>
                        ...
                        <p className="text-xs md:text-sm text-neutral-400 leading-relaxed mb-4 md:mb-8 max-w-[90%] line-clamp-2 md:line-clamp-none">{template.description}</p>
                        
                        <div className="hidden md:grid grid-cols-2 gap-x-4 gap-y-2 mb-8 text-xs text-neutral-400 border-t border-white/20 pt-4">
        ...
        {totalPages > 1 && (
            <div className="col-span-12 bg-black p-8 flex justify-center items-center gap-4 border-b border-white/20">
                <Button 
                    ...
                    variant="outline"
                    className="rounded-none text-white border-white/20 hover:bg-white hover:text-black"
                >
                    Previous
                </Button>
